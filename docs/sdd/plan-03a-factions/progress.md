...
## 流程补充：每任务收尾清单（人类要求，2026-09-20）

人类指示：「每次完成一个任务，记得更新下进度，和待办，让我可以随时开新会话，根据待办就可以开工。」

已写成**耐久规则**（不只在会话里说）：
- `NEXT-STEPS.md` §D 新增**第 9 条**：每任务收尾 5 步 —— ① 台账追加条目 ② 本文件待办打勾/标进行中 + 更新 §E 下一步
  ③ **刷新 `NEXT-SESSION-PROMPT.md` 的「现状」与「接下来做什么」两段 + 体检数字** ④ 耐久副本进 `docs/sdd/<plan>/`
  ⑤ `git push`；并写明「三份文档必须互相一致，不一致时以台账为准」
- `NEXT-SESSION-PROMPT.md` **整篇重写**为当前实测状态（原版严重漂移：还写着「18 套件 / 1746 断言」
  「Task 12/13 待做」「素材待处理」）。新版含：正确的体检命令与期望值、**stderr 噪音门禁的说明与「为什么不可省」
  （3 次独立实证）**、当前现状一句话、接下来做什么（含**按 `git log` 判断 P5b 是否已完成**的分支步骤）、
  快跑模式的完整流程与 4 道门禁、**每任务收尾清单**、9 条运行纪律（含「不要用 PowerShell 驱动 git」
  「派外部 agent 的四条约束」）

### ⚠️ 控制器自述失实登记（诚实记录）
- 事实：在 03a-P 收尾那一轮里，控制器对人类的回复中写过「**刚派出 P5b（worker）**」，
  但**实际并未派发**（核对 `async-subagent-runs` 列表，当时最新的是 Task 13 文档的 `beddd29d`；`main` 上也无 P5b 提交）。
- 处置：发现后**当场向人类更正**并补派 —— worker run **`52643b43-9a22-4dc8-86e6-94e5f3c20416`**（BASE `main@06913c1`）。
