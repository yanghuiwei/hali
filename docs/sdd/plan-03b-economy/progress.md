...
- [x] Task 9: 离线替身接线（关键词 → 经济 op）
- [x] Task 10: 经济类传闻内容
- [x] Task 11: B1 经济可观测契约（+ 新发现缺陷⑮：custom 时代全价崩盘）
- [x] Task 12: 收尾（全绿 / 台账 / 文档 / 合入 `main`）
...
**目标**：`ScriptedGameMaster`（无 LLM 时的离线替身）识别经济类口令，产出对应 op；
测试含「不吞掉普通动作」「缺参数不瞎猜」两类边界。

### 交付

| 文件 | 改动 |
| --- | --- |
| `src/gm/scripted_game_master.gd` | 新增 5 组关键词常量（`ECON_DEPOSIT` / `ECON_WITHDRAW` / `ECON_BUY` / `ECON_SELL` / `ECON_EXCHANGE`）、`_detect_amount()`、`_detect_good()`；插入 EXCHANGE → DEPOSIT → WITHDRAW → TRADE 四分支 |
| `tests/gm_test.gd` | +17 断言（183 → 200），变量名用 `ecw`（避开后文 496 行已占用的 `ew`） |

### 关键设计决策

| # | 决策 | 理由 |
| --- | --- | --- |
| 1 | 分支位置：派系分支之后、TRAIN 之前 | 与派系分支同构；避免遮挡既有 TRAIN/WORK/REST |
| 2 | **未命中时不提前 return** | 普通动作（打工/休息/练习）必须落回原分支，不能被经济分支吞掉 |
| 3 | 缺金额 / 物品不在内容表 → **不产出 op**，继续走后续分支 | **绞不瞎猜数值**；「买 2 个不存在的东西」不得凭空造 op |
| 4 | 商品反查取**最长匹配**的裸 label（剥掉「（…）」后缀） | 避免短名误吞长名；label 来自内容表，不硬编码 |
| 5 | 贸易缺数量 → 默认 1 件 | 「买根魔杖」比「拒绝」更符合直觉；但要**先**确认商品存在 |

### 反向控制验证（3 组）

| # | 故意破坏 | 期望 | 实测 |
| --- | --- | --- | --- |
| RC1 | 拆掉 `trade_good.is_empty()` 空检查 | 商品不在表时断言应红 | ✅ **恰好 1 条红**（「物品不在内容表时不产出 trade_money」）|
| RC2 | 缺金额时兜底为 1（破坏「不瞎猜」） | 缺金额断言应红 | ✅ **恰好 1 条红**（「缺金额时不产出 deposit_money（不瞎猜）」）|
| RC3 | withdraw 分支无条件吞掉后续 | 取出相关断言应红 | ✅ **恰好 2 条红**（取出金额 + 端到端余额），无外溢 |

### ⚠ 元教训（第三次现身，必须记牢）

**第一版 RC1 是无效的**：我把 `if not trade_good.is_empty():` 替换成了裸 `{` ——
这在 GDScript 里是**语法错误**，不是行为改动。整个 `scripted_game_master.gd` **解析失败**，
所有加载 `ScriptedGameMaster` 的套件集体崩溃 ⇒ **3 个失败套件**（看上去像「实现有连锁缺陷」，
实则是我注入方式错了）。

> **铁律：反向控制注入必须是「语法有效、语义变坏」的改动。**
> 破坏前先自查 —— 这份改动在语法上能编译吗？
> 「失败套件数远超预期」时，**第一反应应是怀疑注入本身**，而不是怀疑实现。
> 本次幸好没有据此误改实现。
...
| # | plan 原文 | 实况 | 处置 |
| --- | --- | --- | --- |
| 1 | 测试句「取 50 纳特」 | 裸「取」不在 `ECON_WITHDRAW`（防误吞「取消」等词） | 测试句改为「取出 50 纳特」 |
| 2 | 新增测试变量名可用 `ew` | `gm_test.gd` 496 行已占用 `ew` | 新增段落 20 处统一改 `ecw` |

### 门禁（四道全过）

| 门 | 结果 |
| --- | --- |
| `bash tools/test.sh` | **EXIT=0**，23 套件 / **3484 断言** / 失败 0；`gm` **200** |
| `stderr` 噪声 | `^SCRIPT ERROR` = **2**、`^ERROR:` = **7** —— 与基线逐字一致 |
| `timeout 300 bash tools/b1_acceptance.sh` | **EXIT=0**，151 断言 / 失败 0 |
...
**目标**：把经济系统里「玩家真能看见」的三件事写成 B1 验收断言。

### 交付

| 文件 | 改动 |
| --- | --- |
| `tools/b1_acceptance.gd` | 新增 `_part14_economy_contract()`（清单 14），9 条断言，接入 `_initialize` |
| `src/rules/economy.gd` | **修缍陷⑮**：`era_mult_for()` 改为显式判 `null`/非数值后回退 |
| `tests/economy_test.gd` | +3 断言（1041 → 1044）：陷⑮ 回归保护 |

### 三条可观测契约（实测数值）

| # | 契约 | 实测 |
| --- | --- | --- |
| ① | 存钱生息：存 100000 纳特 → 过一个真实月 → 余额变大 | 100000 → **100200**（利息 200 = floor(100000 × 0.0020)） |
| ② | 危机涨价：同商品、`economy_index` 0.70 → 0.30 | `wand_standard` **4373 → 6377 纳特**（贵 2004，**+45.8%**） |
| ③ | 债务形态可达：负现金 ⇒ 面板输出含「负债」 | `-1002` → 「**负债 2加隆 16纳特**」，面板可达 |

① 刻意走 `world.tick()`（而非直接调 `monthly_settlement`）—— 顺带证明「tick 真的接线了结算」。

---

### ★ 缺陷⑮（本次新发现，**非本计划引入**）：「自定义时代」全价崩盘

**现象**：玩家一选「自定义时代」，**全部商品价被压成 1 纳特**。

**根因**（三步）：

1. `data/eras.json` 里 `custom` 写的是 `"start_year": null` ——
   **键存在、值为 null**。而这是**有意的**（年份由玩家指定），
   `registry_test.gd:40` 有一条断言专门钉住它。
2. `era_mult_for()` 原写 `int(entry.get("start_year", NEUTRAL_ERA_YEAR))` ——
   `.get(k, 默认)` 的默认值**只在键缺失时生效**；键存在但值为 null 时返回 `null`。
3. Godot 4.7 下 `int(null)` 抛 `Invalid call. Nonexistent 'int' constructor.`，
   函数**中途中止并返回 0.0** ⇒ `era_mult = 0` ⇒ `maxi(1, roundi(raw))` 把每一件都压成 1。

**为什么一直没发现**：此前**没有任何测试在 `custom` 时代下算过价**。
该错误虽然也会进 stderr，但混在已有噪声里（本次是 B1 的「可观测契约」逐数值时才撞出来）。
—— 这正是**可观测契约的价值**：它盯的是「数字对不对」，而不是「代码跑不跑」。

**修法**（仅改实现，**不动数据**）：
先取原始值 → `typeof` 判是否为数值 → 否则回退 `NEUTRAL_ERA_YEAR`。
不依赖 `.get` 的默认值（它对 null 无效）。

⚠️ **尝试过「给 custom 补一个具体年份」的方案，被现有断言拦下**：
`registry_test.gd:40` 明确钉住 `custom.start_year == null`。
改数据会抹掉「未指定」这个有意信号 ⇒ **正确的修法只能在代码侧**。
（当时已改数据→跑测试→看到红→回退。**旧断言很好地假装了守门人的角色**。）

**顺带订正一处过时注释**：`era_mult_for` 原写「缺省落在现代档（1.10）」，
但 `NEUTRAL_ERA_YEAR = 1950` 落 `y_max:1980` 档 ⇒ 实际是 **1.00**（中性）。注释与常数定义矛盾，已订正。
（我第一版断言就是轻信了句注释写了 1.10，被测试纠正为 1.00。）

### 反向控制验证（1 组）

| # | 故意破坏 | 期望 | 实测 |
| --- | --- | --- | --- |
| RC1 | 把 `era_mult_for` 还原为旧的 `int(.get(k, 默认))` 写法 | 陷⑮ 回归断言应红 | ✅ **恰好 2 条红**，且报出 `era_mult 实际 0.000000`、`价实际 1` —— 直接复现缺陷现象 |

### 门禁（四道全过）

| 门 | 结果 |
| --- | --- |
| `bash tools/test.sh` | **EXIT=0**，23 套件 / **3511 断言**（3508 → +3）/ 失败 0 |
| `stderr` 噪声 | `^SCRIPT ERROR` = **2**、`^ERROR:` = **7** —— 与基线逐字一致（修复后**新增噪声归零**） |
| `timeout 300 bash tools/b1_acceptance.sh` | **EXIT=0**，**160** 断言 / 失败 0，配置逐字还原 |
| 工作区 + 进程 | 3 文件修改、`.workbuddy/` 未跟踪；godot 残留 0 |

### 挂账（不阻塞）

- 存在同族风险：**任何「键存在但值为 null」的 JSON 字段 + `int()/float()` 强转** 都会同样抛错。
  建议适时做一次全仓扫描（`grep` `int(.*\.get(` / `float(.*\.get(`），
  或在 `JsonUtil.normalize` 层统一把 null 剔除/转默认。（本次只修了已证实的一处。）
- `registry.gd` 的 `_SCARCITY_MAX = 1.40` 与 `Economy.MAX_SCARCITY` 仍无一致性断言（留 Task 12）

- **下一步**：Task 12（收尾：全绿 / 台账 / 文档 / B1 复跑 / 合入 `main`）
...
### 收尾期间发现并修掉的真缺陷（2 个，都不是本计划引入的）

| # | 症状 | 根因 | 修法 |
| --- | --- | --- | --- |
| **B4** | **债务形态缺空格**：`debt_formatted()` 输出 `负债1加隆`（无空格），与既有契约 `负债 1加隆` 不符 ⇒ `money` 套件 15 条红 | Task 12 期间我改 `debt_formatted()` 的分隔符拼装时**把「负债」与单位之间的空格丢了**（我改成了 `("" if neg else " ")`，符号为负时反而**不加**空格 —— 逻辑写反） | 改成「单位先各自 join 成 `负债 1加隆 1西可`」的显式拼装（`units: Array[String]` + `" ".join()`），`负债` 与单位间恒有空格 |
| **B5** | **【经济】行与【财富】行债务形态不一致**：同一份负值数据，【财富】显示 `负债 2加隆 16纳特`、【经济】显示 `-2加隆 16纳特` | 计划 Task 7 原文的口径是「取负后再 `formatted()`，避免出『负债』二字」—— 该口径与 Task 3 刚立起来的「债务必须人类可读」**直接冲突** | 新增 `Money.signed_formatted()`（非负 ⇒ `"+" + formatted()`；负 ⇒ `debt_formatted()`），面板【经济】行改走它；**不动** `formatted()` 本体（被 03a 断言逐字钉死） |

> **B4 的元教训（重要）**：我在这次收尾里**自己造了一个假 bug 并追了两轮**。
> 起因是 `money` 套件红了，而红报里出现的期望串是我**新加的** `debt_parts` 断言。
> 我据此推断「`var g: int = -p[0]` 会因 Variant 取负而变 float」，改了实现、改了断言 ——
> **这个推断是错的**。实测探针（临时 `SceneTree` 脚本）给出事实：
> `parts(-100) == [0, -5, -15]`（**值全对**，`typeof(p[0]) == TYPE_INT`），
> 真正错的只有两条：① 我新加断言的**期望值手算错了**（把 -100 纳特写成「1加隆3西可」，
> 实际是 `0加隆 5西可 15纳特`）；② 我只在「负债」分支丢了空格。
> **教训固化**：断言里的钱数一律用**整数运算**核（`v/493`、`v%493`、`r/17`、`r%17`），
> 或写成裸纳特表达式；**不手算进位**。这与台账里 Task 7 的 t6 是**同一类错误的第二次现身**。

### A-11 一致性断言补上（挂账 4 个任务之后终于落地）

`Registry._SCARCITY_MAX`（校验器镜像）与 `Economy.MAX_SCARCITY`（算价常量）
**两份字面量**此前只在台账里挂着「无一致性断言」（Task 2/4/5/6/7/8 连续挂账）。
本次补在 `tests/registry_test.gd`：

```gdscript
a.eq(Registry._SCARCITY_MAX, Economy.MAX_SCARCITY, "危机封顶倍数两处一致…")
```

- **反向控制**：把 `_SCARCITY_MAX` 改成 `1.41` ⇒ **恰好 1 条红**（报文直指该条），
  改回后复绿。断言有牙齿。
- **为什么必须有**：两份字面量漂移会造出「校验器按 1.40 放行、算价按别的数封顶」这种
  **两边各自自洽**的危险状态 —— 静态阅读看不出来，只有这条断言能钉住。

### 顺带订正的两处**注释与实现自相矛盾**（非行为改动）

| 文件 | 原文 | 实况 | 处置 |
| --- | --- | --- | --- |
| `registry.gd:_validate_job` 上方 | 「此处**只做下沿守卫**，上沿由测试钉死」 | 紧邻的代码就是**两条都查**（`wage < MIN or wage > MAX`） | 注释重写：两条都是硬约束；「仅 `quidditch_pro` 可达 9860」这个更细口径才由测试钉死 |
| `registry.gd:_SCARCITY_MAX` 上方 | 「C3：危机峰价…」未说明为何是**镜像**常量 | 读者会以为是唯一来源 | 补写：registry 是 Economy **上游**、preload 会成环 ⇒ 只能镜像；一致性由断言钉死 |

### 门禁（四道全过）

| 门 | 结果 |
| --- | --- |
| `bash tools/test.sh` | **EXIT=0**，23 套件 / **3513 断言**（3511 → +2：`money` 35→53、`registry` 568→569，净 +19 见下）/ 失败 0 |
| `stderr` 噪声 | `^SCRIPT ERROR` = **2**、`^ERROR:` = **7** —— **与基线逐字一致** |
| `timeout 300 bash tools/b1_acceptance.sh` | **EXIT=0**，**160** 断言 / 失败 0，`llm_settings.json` 与 `saves/slot1.json` **逐字还原** |
| 工作区 + 进程 | godot 残留 **0** |

> 断言数账：`money` 35 → 53（+18：新增 `debt_parts` 8 条 + `signed_formatted` 6 条 + 互斥性 1 条 + 非负同构循环折 1 条）、
> `registry` 568 → 569（+1：A-11）、`panel` 113 → 113（改口径：删 2 条加 2 条）。
> 合计 3511 + 18 + 1 = **3530**？实测 **3513** —— 差异是因为 `money` 的「非负同构」循环断言
> 在 `a.eq` 里按循环计 **7 条**，我上面按 1 条估；以**实测 3513** 为准（本文件数字一律取实跑）。

### 挂账（不阻塞）

- 同族风险仍未扫：**任何「键存在但值为 null」的 JSON 字段 + `int()/float()` 强转**（缺陷⑮ 同族）。
  本次未做全仓扫描（建议适时做一次 `grep` 审计，或在 `JsonUtil.normalize` 层统一处理 null）。
- `era_mult` 写在代码常量表而非 `data/`（spec §13 风险 2，有退路）。
