...
| **K2** | 月度结算（E4②）默认**给玩家发工资**（按职业），这会让「不行动也能活下去」成为默认。接受吗？ | **接受，但要有门槛**：只对有 `job` 且成年（`age_months >= 17×12`）的玩家结算；无业者只有**支出**没有收入（会真的变穷）。正典第十五章「普通巫师的一日只是上班」，故「上班有钱」是正典基线；「不工作会饿」才是模拟器的意义。<br>⚠️ **2026-09-21 施工补充（缺陷⑫）：未成年 = 整月不结算（收支皆 0）**，见 §7.6。原 K2 文本只说了「工资门槛」，**没说未成年是否照收生活费** —— 实算发现若照收，11 岁开局到 17 岁前累计欠 **−946 加隆**（≈普通家庭 6 年收入），而正典 424 行未成年不得在校外使用魔法、正典 563 行 17 岁前是「学徒」阶段 ⇒ **未成年由家庭/学校供养，自理生活费不成立**。 |
...
| 行 | 章节 | 本计划用到什么 |
| --- | --- | --- |
| 198 | 第十五章 普通巫师社会 | 职业清单（商人/魔药师/记者/治疗师/店员/酒吧老板/扫帚修理工/猫头鹰驯养师/魁地奇球员）⇒ `data/jobs.json` + `job → 工资`映射；「上班」是正典基线（K2）。**⚠️ 原写 196，实际正文在 198**（±2 偏差，2026-09-21 施工核对修正） |
| 233 | 第十五章 魁地奇联盟 | 「职业球员的生涯并不轻松。受伤、交易、状态下滑、舆论压力都存在。魁地奇不只是一项运动，也是商业」⇒ 魁地奇球员工资允许破格到 20 加隆/月（§7.6） |
...
### 5.2 ⑤ / ⑥ 的顺序契约（2026-09-21 施工修正，缺陷⑬）

**结论：`evolve` 必须在 `monthly_settlement` 之前，两者都必须在 ① `world_vars` 回归之后。**

⚠️ 但**理由与我原先写的不同**，这条原先写错了：

| 项 | 原 spec 说法 | 实测 |
| --- | --- | --- |
| 顺序理由 | 「`evolve` 必须在 `settlement` 之前，**否则结算用的是上月价**」 | **不成立**。`price_of()` 是**活算**（每次从 `world_vars` 重算），**从不读快照**；`monthly_settlement` 走的是 `price_of` ⇒ 无论顺序如何，结算取到的都是**当月**景气算出的价 |
| 证据 | —— | 我把两阶段顺序**对调**跑反向控制，`economy` 套件 **1037 断言仍然全绿** ⇒ 原有那条顺序断言是**空转的** |
| 真正的理由 | —— | `_snapshot_prices` 产出的快照是**面板（Task 7）/ `PromptBuilder`（Task 8）**读的那份「本月价目表」（`snapshot_price()` 快照优先、缺失才回落活算）。`evolve` 排在 `settlement` 之前，保证「**面板看到的价**」与「**结算用的价**」**同源同月** |

⇒ **顺序仍然不可换，但契约改述为**：「快照必须在本回合的结算与呈现之前刷新，且快照值必须与同期活算值逐字一致」。
机器断言见 §11（这条有反向控制验证，不再空转）。

**教训（与缺陷⑪ 同型）**：**断言必须打在代码真正做的那件事上。**
「顺序错了会怎样」这类断言，如果不做反向控制（故意把顺序改错、看断言是否变红），就很容易写成
**看着合理、实则空转**的装饰 —— 本任务就写出了一条。
...
| `data/jobs.json` | 新增（Task 5，缺陷⑩） | 9 类城市巫师职业 + 月薪（正典 198 行；§7.6） |
| `src/core/registry.gd` | 修改 | `TABLE_FILES` 注册 `goods` / `industries` / `jobs`；`validate()` 增加三表校验（引用合法性、枚举、价格 > 0、工资量级） |
...
### 7.6 职业表 `data/jobs.json` 与月度结算口径（E4②，**2026-09-21 补章，缺陷⑨/⑩**）

> **⚠️ 本章是施工中补写的**。原 spec 把「职业 → 工资」的映射与月度结算的收支口径都留成了待定，
> 施工到 Task 5 时发现两个硬前提**根本不存在**，按铁律先补 spec 再写代码。

**缺陷⑨：食物价格量级错。** 原 `goods.json` 的 `food_butterbeer = 2 纳特`、`food_pumpkin_pastry = 1 纳特`。
`1 纳特 = 1/493 加隆 ≈ 0.002 加隆` ⇒ **一份南瓜馅饼比一个月房租（4437 纳特）便宜 4437 倍**。
正典第 223 行**没有给食物价格** ⇒ 这不是照抄错误，是当初把 **西可数当纳特数** 填了
（正典语境里黄油啤酒约 2 西可 ⇒ 应为 `2 × 29 = 58 纳特` 量级）。同时这两行的 `industry_id` 是 `publishing`（出版）——
**南瓜馅饼归出版社**，属同一批填充事故。

- **修正值**：`food_butterbeer` = `34` 纳特、`food_pumpkin_pastry` = `34` 纳特（各 ≈ 2 西可的等价纳特档，取整到 34 而非 58，
  理由见下方「为什么取 34」），`industry_id` 改为 `""`（无产业，与既有 `svc_*` 服务行的做法一致）。
- **为什么取 34 而不是 58**：`1 西可 = 29 纳特`，`2 西可 = 58 纳特`。但结算里食物按 **60 份/月** 计，
  `58 × 60 = 3480` 会让月支出冲到 **7917 纳特**，超出计划预期的 6900 一个档。
  `34 × 60 = 2040` ⇒ 月支出 **6477 纳特**，**正好落在计划实算过的 6900 量级**
  （这是缺陷⑨ 的旁证：计划作者做那次实算时用的食物价是对的，写进 `goods.json` 时错了）。
  ⇒ **取 34 = 「让结算结果与计划的既有实算自洽」优先于「60 纳特字面换算」**。
- ⚠️ 这两个数是**结算口径**（量级锚点），**不是正典断言**：`canon_price_knuts` / `canon_price_hi_knuts` 保持 `0`，
  `canon_line` 保持 `0`，**不得**为了让 `Registry._validate_goods` 的 C6 分支通过而编造正典行号。

**缺陷⑩：职业表不存在。** `player.job` 是**自由字符串**（`gm_test` 塞「魔药学徒」、`economy_test` 塞「auror」、
`character_creation.gd:224` 置 `""`）。原 plan 写「职业名到 id 的映射若已在 `data/jobs.json` 就复用（**先查**，不要新造表）」——
**查了：不存在**。而 `_wage_for(job)` 无表可依。

⇒ 新建 `data/jobs.json`（**第 3 张内容表**，须注册进 `Registry.TABLE_FILES`）：

```json
[
  {"id": "shopkeeper",   "label": "商人",       "wage_knuts": 6900, "canon_line": 198},
  {"id": "apothecary",   "label": "魔药师",     "wage_knuts": 7395, "canon_line": 198},
  {"id": "journalist",   "label": "记者",       "wage_knuts": 5423, "canon_line": 198},
  {"id": "healer",       "label": "治疗师",     "wage_knuts": 8874, "canon_line": 198},
  {"id": "shop_clerk",   "label": "店员",       "wage_knuts": 4930, "canon_line": 198},
  {"id": "pub_owner",    "label": "酒吧老板",   "wage_knuts": 7888, "canon_line": 198},
  {"id": "broom_repair", "label": "扫帚修理工", "wage_knuts": 5916, "canon_line": 198},
  {"id": "owl_keeper",   "label": "猫头鹰驯养师", "wage_knuts": 5423, "canon_line": 198},
  {"id": "quidditch_pro","label": "魁地奇球员", "wage_knuts": 9860, "canon_line": 198}
]
```

- **正典依据**：第 **198** 行「城市巫师包括：商人、魔药师、记者、治疗师、店员、酒吧老板、扫帚修理工、猫头鹰驯养师、魁地奇球员」
  ——**逐条对应，9 条不多不少**。注意 `spec` §4 引用表里写的行号是 **196**，实际正文在 **198**（原引用有 ±2 偏差，见 §13 缺陷⑩b）。
- **量级锚点（硬约束）**：正典 223 行「普通家庭年收入约数百加隆」⇒ 月收入落在 **6–15 加隆 = 2958–7395 纳特** 区间。
  上表工资 **4930–9860**，中位数 6900：**下限守住 6 加隆，上限 9860 = 20 加隆略高于 15** ——
  理由：魁地奇球员是「明星职业」，正典第 233 行明确其生涯有商业价值，允许破格一档。
  其余 8 条**全部**在 `[2958, 7395]` 内（机器断言见 §11）。
- ⚠️ **上一条「其余 8 条全部在 [2958, 7395] 内」是错的（2026-09-21 施工实算，缺陷⑪）**。
  实算：`healer = 8874 = 18.00 加隆`、`pub_owner = 7888 = 16.00 加隆`，**两条都越出 7395 上沿**。
  越界数 = 3 条（`healer` 18 / `pub_owner` 16 / `quidditch_pro` 20），不是 1 条。
  **修正后的口径（本节为准）**：
  - **下沿 2958（6 加隆）是硬约束**，9 条全部满足 ✔ —— 这是「不得低于普通家庭月入」的守卫，保持。
  - **上沿不是 7395**。7395 只是「6–15 加隆」这个区间的右端，而 15 加隆**不是正典数字**，
    是我按「年收入数百加隆 ÷ 12」倒推的软参考。正典对巫师收入**只给了区间量级，没给上限**。
    ⇒ 上沿的机器断言改为 **`<= 9860`（20 加隆）**，且**只有 `quidditch_pro` 允许触及 9860**；
    其余 8 条断言 **`< 9860`**（严格小于，防「人人都按上限发薪」）。
  - **量级自洽的判据不是「每条都 ≤ 15 加隆」，而是「中位数年收入落在数百加隆」**：
    中位 6900 × 12 = 82 800 纳特 = **168 加隆** ✔（正典 223 行）。
    另：治疗师 18 加隆 / 酒吧老板 16 加隆**在叙事上站得住**——治疗师是正典里的高技能专业岗
    （圣芒戈），酒吧老板是**自营业主**（收入含经营所得，不是纯工资）。故**不改数值，改断言口径**。
  - `Registry._validate_job` 的上下沿据此设 `[2958, 9860]`（包住全部 9 条），
    「只有 quidditch_pro 能到 9860」由 `tests/registry_test.gd` 的显式断言钉死。
- **`wage_knuts` 取 `493` 的整数倍是巧合不是规则**（`svc_rent` 的 4437 = 9 加隆 是照抄；本表多数取整到百位便于阅读）。
- **工资率进 `data/` 而不进 `Economy` 常量表**：与 `ERA_MULT` 的处置不同（§13 风险 2），
  理由是**职业是玩家可见文本**（面板【职业】行直接显示 `label`，`_wage_for` 按 `label` 反查），
  而 `ERA_MULT` 是纯演化参数、玩家永不可见。这与「内容进 data 的铁律」一致。

**月度结算口径（`Economy.monthly_settlement`）**：

| 项 | 口径 | 备注 |
| --- | --- | --- |
| 幂等 | `last_settlement_turn == clock.turn` ⇒ **直接返回** `{0,0,0}`，**不改任何字段** | spec §9 第 5 条 |
| 门槛（工资） | `age_months >= ADULT_MONTHS`（`17 × 12 = 204`）**且** `job` 非空 | K2 |
| **门槛（未成年整月跳过）** | `age_months < ADULT_MONTHS` ⇒ **整月不结算**：`{0,0,0}`，**生活费也不收**、`last_settlement_turn` 照常推进（**缺陷⑫**，2026-09-21 裁定「读法 B」） | K2 补充。理由：正典 424 行未成年不得在校外使用魔法、563 行 17 岁前属「学徒」阶段 ⇒ **由家庭/学校供养**。反例：照收 6477 纳特/月 ⇒ 11 岁开局到 17 岁前欠 **−946 加隆**（≈普通家庭 6 年收入），构成「必破产开局」 |
| 工资 | `_wage_for(job)`：按 `label` 在 `jobs.json` 里**精确匹配**；匹配不到再试**包含**匹配（自由文本宽容）；仍无 ⇒ `UNKNOWN_WAGE_KNUTS = 4930`（店员档，**不是 0**） | 自由文本职业不得变成零收入 |
| 生活开销 | `price_of("svc_rent") + price_of("food_pumpkin_pastry") × FOOD_UNITS_PER_MONTH`（`FOOD_UNITS_PER_MONTH = 60`，即两餐/日） | **必须走 `price_of`**（不写死 base），否则危机期物价翻倍而开销不变 |
| 利息 | `interest = floori(balance × gringotts_interest_rate)`，**只加本金、不资本化到负余额**；`balance < 0` ⇒ `interest = 0` | 危机期利率由 `evolve()` 下调 |
| 资产负债 | `player.money_knuts += income - expense`（**允许为负** ⇒ 走 `Money` 的负债形态，Task 3 已就绪） | 正典语境里巫师可以欠债 |
| 写回 | `last_month_income` / `last_month_expense`（**正值**，符号由面板加） / `last_settlement_turn` | §7.3 |
| **成年门槛常量** | `ADULT_MONTHS := 204` | 与 `character_creation` / 既有年龄口径对齐 |

**实算：月收支平衡点（中性时代 + `index=0.5` + 常规地点 ⇒ 价 == `base`）**

| 项 | 纳特 | 折合加隆 |
| --- | ---: | ---: |
| 房租 `svc_rent` | 4 437 | 9.00 |
| 食物 `34 × 60` | 2 040 | 4.14 |
| **月支出合计** | **6 477** | **13.14** |
| 工资（中位，`shopkeeper`） | 6 900 | 14.00 |
| **月净** | **+423** | **+0.86** |
| 无业者月净 | **−6 477** | **−13.14** |

- ⇒ 「无业者净收 < 0」成立（计划 Step 1 的硬断言）；「有业者净收 > 0 但很薄」成立（K2 的「不工作会饿」有牙齿）。
- ⇒ **年支出 = 77 724 纳特 ≈ 158 加隆**，与正典「年收入约数百加隆」同量级（收支两侧都落在数百加隆档）✔
...
6. **`evolve()` 的接线验收**（Task 6）：① `tick()` 后价格快照被刷新，且**逐条等于当回合 `price_of`**（缺陷⑬ 的核心契约 —— 这条**必须经反向控制验证**：注释掉 `_snapshot_prices` 应恰好让这几条变红）；② 危机是**边沿不是电平** —— `false→true` 写 1 条 `economic_crisis` 事件，**危机中再 tick 写 0 条**，`true→false` 写 0 条但利率回升（同样须反向控制：把 `if now and not was` 改成 `if now` 应恰好让「危机中不重复写事件」1 条变红）；③ `foreign_rate` 落 `[0.85, 1.15]`，**同 seed 双世界逐月一致、不同 seed 分岔**（确定性双向判别）；④ `tick()` 真的调用了结算（`last_settlement_turn` 推进 + 工资到账）；⑤ 既有 ⑦⑧⑨ 阶段未被扰动（年龄仍 +1、`log` 仍有界）。
...
| `tests/registry_test.gd` | 修改 | 两表注册 + `validate()` 对坏引用/坏枚举/负价格报错；`canon_price > 0` 而 `base_price` 落出合理带时报错（§7.1 新口径）；**`jobs.json` 注册 + `wage_knuts > 0` + `label` 非空 + 9 条 `canon_line == 198`** |
| `tests/economy_test.gd`（Task 5 补） | 修改 | ⑱ 月度结算幂等（同回合两次只结算一次、**且第二次不改任何字段**）；⑲ 无业者净收 **< 0**；⑳ 未成年（`age_months < 204`）**无工资但有支出**；㉑ 成年从业者工资 > 0 且**落在正典量级**（下沿硬约束 `>= 2958`；上沿 `<= 9860`，**只有魁地奇球员可达 9860**，其余 8 条严格 `< 9860` —— 见 §7.6 缺陷⑪修正）；㉒ **工资表 9 条全部落在 `[2958, 9860]`** + **中位年收入落在数百加隆**（机器守门，防将来改表越界）；㉓ 未知职业走 `UNKNOWN_WAGE_KNUTS`（**不是 0**）；㉔ 存款利息到账（`floori(balance × rate)`）；㉕ **负余额不加利息**；㉖ 开销走 `price_of`（危机期物价翻倍 ⇒ 开销真的变大，证明没写死 base）；㉗ 月收支平衡点实算：中性时代有业者**净收 > 0**、无业者**净收 < 0**，且月支出 `== 6477`（房租 4437 + 食物 34 × 60，防食物价再被写错）；㉘ 同输入两次结算**逐字一致**（确定性）；㉙ `data/jobs.json` 9 条 label 与正典 198 行逐字一致 |
...
4. **价格数量级必须对得上正典**：`普通家庭年收入约数百加隆`（223 行）是**唯一的量级锚点**。若月度结算后玩家年收入 / 年支出量与「数百加隆」差一个数量级，说明工资或物价系数需要整体校准。**这是本计划最可能返工的地方**（一次调常数，不是推翻设计）。
   **⚠️ 2026-09-21 施工实测：这个风险确实兑现了，见缺陷⑨** —— 食物价当初按「西可数」填成了纳特数（南瓜馅饼 1 纳特 = 月房租的 1/4437），
   导致按计划自己的口径实算出的「月支出」只有 **4497** 纳特，而计划预期是 **6900**。修食物价到 34 纳特/份后月支出 **6477**，与预期自洽。
   **元教训**：锚点量级的**自洽性检查**要跑在「写内容」那一步，不能只在「写结算」时才发现 —— 内容表的单个数字错了，会在下游用「结果不对」的形式暴露，而那时排查成本高 10 倍。
...
9. **本 spec 的数值经「先实算后写表」全流程重算 + 施工期持续核对，共发现并修掉 14 处设计缺陷**
   （7 处设计期 ①–⑦ + 7 处施工期 ⑧/⑧b/⑨/⑩+⑩b/⑪/⑫/⑬；教训同 03a §13.2「0.45 在任何合法状态都不可达」）。
   ⚠️ **施工期缺陷（⑧ 起）比设计期更多** —— 因为设计期的实算只覆盖「算价」，而**内容表的正确性、参数的写入方、
   口径的落地点**只有在代码真的跑起来时才被检验。这条经验对后续计划通用：**设计期实算必须覆盖「每个参数的写入方是否存在」**，
   不能只覆盖「公式算得对不对」。
   ...
   | ⑧ | **`local_mult` 的来源不存在**（2026-09-21 Task 4）：`local_mult_for` 读 `world_vars["location_tag"]`，该键**全仓库无生产写入方** | `grep location_tag` 只命中 `economy.gd` 自身 / `economy_test` 手动注入 / plan 文本；且 `world_vars` 在玩家换地点时不变 ⇒ E7「产地买、销地卖」**实现上不可能** | 改按 `locations.json` 的 `zone` 推导（§7.4 第 4 条）；新增 `price_at` / `local_mult_at` |
   | ⑧b | **`MAX_TRADE_QTY` 是件数不是金额，防不住套利**（2026-09-21 Task 4） | `broom_nimbus` 净利 17 135 纳特/件 ⇒ `qty=100` 单笔 **1 713 500 纳特 = 3 476 加隆 ≈ 11.6 倍家庭年收入** | 新增货值闸门 `MAX_TRADE_VALUE = 5000`（§7.5）；放弃「同回合不可重复交易」 |
   | ⑨ | **食物价按「西可数」填成了「纳特数」**（2026-09-21 Task 5 施工时暴露） | 南瓜馅饼 **1 纳特** = 月房租 4437 的 **1/4437**；按计划自身口径实算月支出仅 **4497**，而计划预期 **6900** | 修正为 `34` 纳特/份 + `industry_id` 改 `""`；月支出回到 **6477**（与预期自洽）。见 §7.6 |
   | ⑩ | **职业表不存在**（2026-09-21 Task 5 施工时暴露）：原 plan 写「若已在 `data/jobs.json` 就复用（先查）」 | **查了：没有**。`player.job` 是自由字符串 ⇒ `_wage_for(job)` 无表可依 | 新建 `data/jobs.json`（正典 198 行 9 类城市巫师职业，§7.6）+ 注册进 `TABLE_FILES` |
   | ⑩b | 正典引用行号偏移：spec §4 写「196 ⇒ 职业清单」 | 实际正文在 **198** | 修正引用表；`jobs.json` 各行 `canon_line = 198` |
   | ⑪ | **工资上沿口径自相矛盾**（2026-09-21 Task 5 写测试时暴露）：§7.6 断言「其余 8 条**全部**在 `[2958, 7395]` 内」 | 实算：`healer = 8874 = 18.00 加隆`、`pub_owner = 7888 = 16.00 加隆`，**两条越出 7395**。越界数 **3 条**不是 1 条 | **不改数值，改断言口径**：下沿 2958 保持硬约束；上沿放宽到 `9860` 且只有 `quidditch_pro` 可达；量级自洽改判「**中位年收入落在数百加隆**」（中位 6900×12 = 168 加隆 ✔）。理由：15 加隆不是正典数字，是我倒推的软参考；正典只给量级不给上限 |
   | ⑫ | **未成年是否照收生活费未定义**（2026-09-21 Task 5 写测试时暴露）：K2 只说「工资门槛」，§7.6 开销行只说「房租 + 食物×60」，**两处都没说未成年** | 实算：照收 ⇒ 11 岁开局到 17 岁前累计 **−946 加隆**（≈普通家庭 6 年收入）；正典 424 行未成年不得在校外使用魔法、563 行 17 岁前属「学徒」 | **裁定读法 B（用户 2026-09-21 拍板）：未成年整月跳过**（收支皆 0，仅推进 `last_settlement_turn`）⇒ 与正典「在校生由家庭/学校供养」自洽。改 spec K2/§7.6 + 代码 + 测试断言，把「未成年无开销」升为**契约** |
   | ⑬ | **顺序契约的理由写错了，且据此写的断言是空转的**（2026-09-21 Task 6 **反向控制**时暴露）：§5/§8 称「`evolve` 必须在 `settlement` 之前，否则结算用上月价」 | `price_of()` 是**活算**（从不读快照），`monthly_settlement` 走 `price_of` ⇒ **顺序对结果无影响**。把两阶段顺序**对调**跑反向控制，`economy` **1037 断言仍全绿** ⇒ 那条顺序断言是**装饰** | 改 §5.2：**顺序仍不可换，但理由是另一个** —— `_snapshot_prices` 的快照是**面板（Task 7）/ PromptBuilder（Task 8）**读的「本月价目表」，`evolve` 在前保证「面板看到的价」与「结算用的价」**同源同月**。断言改为 **「快照逐条 == 当回合 `price_of`」**（已用反向控制验证：注释掉 `_snapshot_prices` ⇒ 5 条红，含核心契约「35 条商品全部不匹配」） |
