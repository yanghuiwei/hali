...
> ⚠️ **顺序不可换**（spec §8）：`evolve` 必须在 `settlement` 之前。
> **⚠️ 2026-09-21 施工修正（缺陷⑬）：原写「否则结算用的是上月价」是错的 —— 对当前实现不成立。**
> 实测：`price_of()` 是**活算**（每次从 `world_vars` 重算），**从不读快照**；
> 故 `settlement` 无论排在 `evolve` 前还是后，取到的都是**当月**景气算出的价。
> 我把两阶段顺序对调跑反向控制，`economy` 套件 **1037 断言仍然全绿**（断言是空转的）。
> ⇒ **顺序仍不可换，但真正的理由是另一个**：`_snapshot_prices` 的结果是
> **面板（Task 7）/ `PromptBuilder`（Task 8）** 读的那份「本月价目表」，
> 而 `snapshot_price()` 是快照优先、缺失才回落活算。
> `evolve` 排在 `settlement` 之前，是保证「面板看到的价」与「结算用的价」**同源同月**。
> 断言相应改为对**快照与活算一致性**的检查（这条真的有牙齿），见 Step 1。
> 两者仍都必须在 ① `world_vars` 回归之后（景气是本回合新算出来的）。
